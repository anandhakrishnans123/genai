    ...
    if uploaded_file is not None:
        ...
        if st.button("Convert Image to CSV"):
            try:
                ...
                prompt = "Extract data from the uploaded image,check if the image is skwed, if it is skwed unskew it and extract the data including handwritten  text,numbers and convert it to a CSV format. If possible, identify the type of data (e.g., names, dates, numbers) and structure the CSV accordingly. Also only give out the output table no other specific information is required"
