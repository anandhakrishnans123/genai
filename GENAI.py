        ...
        if file_type in ["image/png", "image/jpg", "image/jpeg"]:
            ...
            if st.button("Generate Content from PDF"):
                try:
                    ...
                    st.write(csv_result)
                    st.download_button(label="Download CSV",data=csv_result,  # CSV data as byte contentfile_name="downloaded_data.csv",  # Suggested file name for downloadmime="text/csv"  # MIME type for CSV files)
